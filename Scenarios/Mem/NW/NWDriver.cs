...
namespace Transactions.Scenarios.Mem.NW
{
    public class NWDriver
    {
        ...
        public NWDriver(InitNodeFactory initNodeFactory, MaterializedLocksTMFactory tmFactory)
        {
        ...
        public void Run(string name)
        {
            ...
            var driver = new TxDriver(
                ...
                (network, clock, random, address, shardLocator, appLocator) => new NWAppNode(this.tmFactory, network, clock, random, address, shardLocator, appLocator, (long)backoffCapUs, attemptsPerIncrease),
        ...
        public void ExploreDynamics(string name, Microsecond duration, int fromClients, int toClients, int step)
        {
            using (var writer = new StreamWriter(name, true))
            {
                for (var i=fromClients;i<=toClients;i+=step)
                {
                    ...
                    var stat = this.Run(Consts.INTRA_DC_NETWORK, Consts.SLOW_SSD, i, duration);
        ...
        private string Run(IOSpec networkSpec, SSDSpec ssdSpec, int clientCount, Microsecond duration)
        {
            var backoffCapUs = ssdSpec.fsync.value * 5;
            var attemptsPerIncrease = 4;
