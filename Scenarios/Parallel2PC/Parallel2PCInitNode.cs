...
{
    public class Parallel2PCInitNode : Node, IInitNode
    {
        private class InitTx : DbNode.IRWTx
        {
            ...
            public ICloneable Execute(DbNode.IRWStorage storage)
            {
                ...
                {
                    storage.Write(key, new Records.Account3
                    {
                        CurrValue = this.values[key],
                        Version = 0
                    });
                }
        ...
        public Parallel2PCInitNode(IEndpoint network, IClock clock, IRandom random, string address, Func<string, string> shardLocator)
